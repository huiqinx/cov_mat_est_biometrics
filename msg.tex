...
\begin{align*}
\label{eq:iteration}
%\hat{w}_{\tau}^{(k)} = \frac{2}{p(p-1)}\sum_{1\leq k< j \leq p} \frac{\hat{w}_{\tau}^{(k-1)} f(\bs{X}_j,\bs{X}_k \mid \bs{\xi}_{\tau})}{\sum_{l=1}^D\hat{w}_l^{(k-1)} f(\bs{X}_j,\bs{X}_k\mid \bs{\xi}_l)}
